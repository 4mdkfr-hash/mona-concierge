  ...
  return (
    <div className="p-6 space-y-8 max-w-3xl">
      ...
      <section className="bg-carbon border border-graphite rounded-2xl p-6 space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div>
            <div className="text-sm text-ivory">Plan: <span className="font-medium">€200 / month</span></div>
            <div className="text-xs text-fog mt-0.5">
              Status:{" "}
              <span className={
                subscriptionStatus === "active" ? "text-emerald-400" :
                subscriptionStatus === "trialing" ? "text-gold-400" :
                subscriptionStatus === "past_due" ? "text-red-400" :
                "text-fog"
              }>
          ...
          {subscriptionStatus === "active" || stripeCustomerId ? (
            <button
              ...
              className="px-4 py-2 rounded-xl text-xs font-semibold bg-gold-400/10 text-gold-400 hover:bg-gold-400/20 transition-all disabled:opacity-50"
            >
              {billingLoading ? "…" : "Manage subscription"}
            </button>
          ) : (
            <button
              onClick={handleSubscribe}
              disabled={billingLoading}
              className="px-4 py-2 rounded-xl text-xs font-semibold bg-gold-400 text-void hover:bg-gold-500 transition-all disabled:opacity-50"
            >
              {billingLoading ? "…" : "Subscribe — €200/mo"}
            </button>
          )}
        </div>
      </section>
